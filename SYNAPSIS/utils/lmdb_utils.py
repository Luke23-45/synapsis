...
def calculate_lmdb_map_size_gb(
    ...
    # Log the calculation (no more debug prints)
    logger.info(f"LMDB map size calculation: {num_episodes} episodes -> {clamped_gb:.1f} GB")
    
    # [SOTA FIX] Round up to nearest 0.5 GB and ensure we ENFORCE the minimum
    # to avoid the round(0.25) -> 0.0 bug.
    rounded_gb = np.ceil(clamped_gb * 2) / 2
    
    # Final safety clamp
    rounded_gb = max(rounded_gb, min_size_gb)
    
    logger.debug(f"LMDB map size: {num_episodes} episodes -> {rounded_gb:.1f} GB")
    
    return float(rounded_gb)
...
def check_disk_space(path: str, required_gb: float) -> bool:
    """Check if there is enough free space on the disk containing path."""
    import shutil
    total, used, free = shutil.disk_usage(os.path.dirname(os.path.abspath(path)))
    free_gb = free / (1024**3)
    if free_gb < required_gb:
        logger.error(f"⚠️ LOW DISK SPACE: Required {required_gb:.2f}GB, but only {free_gb:.2f}GB available on drive.")
        return False
    return True
...
def open_lmdb_env(
    ...
    if not readonly:
        if not check_disk_space(path, map_size_gb):
            raise IOError(f"Insufficient disk space to allocate {map_size_gb}GB LMDB buffer at {path}")
